...
export default function SearchPage() {
  ...
  const {
    ...
    isError: usersError,
    refetch: retryUsers,
  } = useSearchUsersQuery(q, { skip: !q });
  const {
    data: postsData,
    isLoading: postsLoading,
    isError: postsError,
    refetch: retryPosts,
  } = useSearchPostsQuery({ q }, { skip: !q });
  const {
    data: tagsData,
    isLoading: tagsLoading,
    isError: tagsError,
    refetch: retryTags,
  } = useSearchHashtagsQuery(q, { skip: !q });
  const {
    data: groupsData,
    isLoading: groupsLoading,
    isError: groupsError,
    refetch: retryGroups,
  } = useSearchGroupsQuery({ q }, { skip: !q });
  const {
    data: challengesData,
    isLoading: challengesLoading,
    isError: challengesError,
    refetch: retryChallenges,
  } = useSearchChallengesQuery({ q }, { skip: !q });
    ...
    return (
      <div className="mx-auto max-w-[600px]">
        {recentSearches.length > 0 ? (
          <div>
            <h2 className="mb-3 font-display text-lg font-bold tracking-tight">Recent searches</h2>
        ...
        <TabsContent value="people" className="mt-4 flex flex-col gap-2">
          {usersError ? (
            <Empty>
              ...
              <EmptyDescription>Couldn&apos;t search for people. Try again.</EmptyDescription>
              <button
                onClick={retryUsers}
        ...
        <TabsContent value="posts" className="mt-4 flex flex-col gap-4">
          {postsError ? (
            <Empty>
              ...
              <EmptyTitle>Search failed</EmptyTitle>
              <EmptyDescription>Couldn&apos;t search posts. Try again.</EmptyDescription>
              <button
                onClick={retryPosts}
                className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-brand-teal hover:underline"
              >
            ...
            <div className="flex flex-col gap-4">
              {[1, 2].map((i) => (
                <div key={i} className="h-48 rounded-2xl border bg-[var(--bg-elevated)] shimmer" />
              ))}
            </div>
          ) : posts.length > 0 ? (
        ...
        <TabsContent value="groups" className="mt-4 flex flex-col gap-2">
          {groupsError ? (
            <Empty>
              ...
              <EmptyTitle>Search failed</EmptyTitle>
              <EmptyDescription>Couldn&apos;t search groups. Try again.</EmptyDescription>
              <button
                onClick={retryGroups}
                className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-brand-teal hover:underline"
              >
        ...
        <TabsContent value="challenges" className="mt-4 flex flex-col gap-2">
          {challengesError ? (
            <Empty>
              ...
              <EmptyTitle>Search failed</EmptyTitle>
              <EmptyDescription>Couldn&apos;t search challenges. Try again.</EmptyDescription>
              <button
                onClick={retryChallenges}
                className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-brand-teal hover:underline"
              >
                <RefreshCw className="size-3.5" /> Try again
              </button>
        ...
        <TabsContent value="hashtags" className="mt-4 flex flex-col gap-2">
          {tagsError ? (
            <Empty>
              ...
              <EmptyTitle>Search failed</EmptyTitle>
              <EmptyDescription>Couldn&apos;t search hashtags. Try again.</EmptyDescription>
              <button
                onClick={retryTags}
                className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-brand-teal hover:underline"
              >
