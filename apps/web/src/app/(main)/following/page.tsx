...
import { useGetFollowingQuery } from "@/redux/api/userApi";
import { useFollowActions } from "@/hooks/useFollow";
import { motion } from "framer-motion";
import { AlertCircle, RefreshCw, UserMinus } from "lucide-react";
...
export default function FollowingPage() {
  ...
  const { data, isLoading, isError, refetch } = useGetFollowingQuery(
  ...
  const handleUnfollow = async (userId: string) => {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "Failed to unfollow user"));
    }
  };
    ...
    <div className="space-y-6">
      ...
      {isError ? (
        <GlassCard className="p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground">Couldn&apos;t load following list.</p>
